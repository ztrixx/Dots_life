    ...
    def __init__(self, x, y, gap, start_seed=None, color=None):
        ...
        if start_seed is None:
            start_seed = str({'size': randint(10, 20),
                              ...
                              'color': color,
                              'character': [choice(['Peaceful', 'Agressive']), choice(['Smart', 'Not smart'])]})
    ...
    def go_to_or_out(self, object):
        ...
        if object.type == 'dot':
            if object != self and object.seed['color'] != self.seed['color']:
                if self.seed['character'][0] == 'Agressive':
                    if object.seed['size'] > self.seed['size']:
                        if self.seed['character'][1] == 'Smart':
                            ...
                            self.move_to_or_out_object(object, True)
                            if self.rect.colliderect(object.rect):
                                object.seed['size'] += self.seed['size'] // 2
                                object.rect.size = (object.seed['size'], object.seed['size'])
                                return False
                    else:
                        self.move_to_or_out_object(object, True)
                        if self.rect.colliderect(object.rect):
...
class Food:
    def __init__(self, size, color, gap, poison):
        self.poison = poison
        self.gap = gap
        self.size = size
        self.color = color
